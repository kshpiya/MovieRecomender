...
public class WebSecurityConfig {
	 ...
	 * .requestMatchers("/api/create").hasAnyAuthority("ADMIN")
	 * .requestMatchers("/api/update/**").hasAnyAuthority("ADMIN")
	 * .requestMatchers("/api/remove/**").hasAuthority("ADMIN")
	 * .requestMatchers("/api/rate/**").hasAuthority("USER")
	 * .anyRequest().authenticated() ) .formLogin(login -> login.permitAll())
	 * .logout(logout -> logout.permitAll()) .exceptionHandling(eh ->
	 * eh.accessDeniedPage("/403")) ;
	 * 
	 * return http.build(); }
	 */
}